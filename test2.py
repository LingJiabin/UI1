...
class RequestApi(object):
    def __init__(self, appid, secret_key, upload_file_path, pre_file_path):
        self.appid = appid
    ...
    def all_api_request(self):              #总任务函数集中
        # 1. 预处理
        ...
        i = 11;
        k = len(T_list)
        T_path = self.pre_file_path + r'/News.txt'
        if os.path.exists(T_path):
            os.remove(T_path)       #文件存在则删除
...
def Translate_A(apid, sk, pre_fp):              #提供调用的接口
    APP_ID = apid                           #由讯飞语音提供
    SECRET_KEY = sk                         #由讯飞语音提供
    file_path = pre_fp + r"\News.wav"
    api = RequestApi(appid=APP_ID, secret_key=SECRET_KEY, upload_file_path=file_path, pre_file_path= pre_fp)
    api.all_api_request()

if __name__ == '__main__':
    APP_ID = "5e5267cc"
    SECRET_KEY = "1b0ac63fa1dca15f894e6a427022a70f"
    pre_fp = r'data\14'
    file_path = pre_fp + r"\News.wav"
    api = RequestApi(appid=APP_ID, secret_key=SECRET_KEY, upload_file_path=file_path, pre_file_path= pre_fp)
    api.all_api_request()
